...
class Lws {
   ...
   */
  start () {
    this.options.stack = util.expandStack(this.options.stack, this.options)
    let cli
    try {
      cli = util.parseCommandLineOptions(util.getOptionDefinitions(this.options.stack), this.options)
    } catch (err) {
    ...
    storedOptions.stack = util.expandStack(storedOptions.stack, optionsSoFar)
