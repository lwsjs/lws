...
class Lws {
  ...
  start () {
    ...
    const optionsSoFar = util.deepMerge({ port: 8000 }, this.options, cli.options._all)
    const loadConfig = require('config-master')
    const storedOptions = loadConfig('local-web-server', { filename: optionsSoFar['config-file'] })
    storedOptions.stack = util.expandStack(storedOptions.stack)
    this.options = util.deepMerge({ port: 8000 }, this.options, storedOptions, cli.options._all)
