...
class Lws {
  ...
  constructor (options) {
    ...
    this._optionDefinitions = cli.optionDefinitions
  }

  // parseCommandLineOptions () {
  //   const stackOptions = commandLineArgs({ name: 'stack', multiple: true })
  //   if (stackOptions.stack.length) {
  //     this.loadFeatures()
  //   }
  //   const commandLineArgs = require('command-line-args')
  //   try {
  //     const options = commandLineArgs(this._optionDefinitions)
  ...
  expandStackOptions () {
    if (this.options.stack && this.options.stack.length) {
      this.options.stack
        .forEach((featurePath, index) => {
          if (typeof featurePath === 'string') {
            this.options.stack[index] = loadFeature(featurePath)
          }
        })
    } else {
 ...
 */
// function parseFeaturePaths () {
//   const featurePaths = []
//   const featureIndex = process.argv.indexOf('--stack')
//   if (featureIndex > -1) {
//     for (let i = featureIndex + 1; i < process.argv.length; i++) {
