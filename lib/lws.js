...
class Lws {
  ...
  start () {
    ...
    const builtInDefinitions = require('./cli-data').optionDefinitions
    let cliOptions = commandLineArgs(builtInDefinitions, { partial: true })
    ...
    const stack = Stack.create([
      cliOptions._all.stack,
      storedOptions.stack,
      this.internalOptions.stack
    ].find(s => s && s.length))
    ...
    const allDefinitions = [ ...builtInDefinitions, ...stackDefinitions ]
