class LwsCli {
  ...
  start (options, argv) {
    try {
      const server = this.execute(options, argv)
      if (server) {
        server.on('error', err => {
          this.printError(err)
        })
  ...
  getOptions (initialOptions, argv) {
    ...
    options.moduleDir.push('.')

    const stack = MiddlewareStack.from(firstDefinedStack, options)
    ...
    const viewModule = View.load(firstDefinedView) || require('./view/cli-view')
  ...
  execute (options, argv) {
    ...
    if (options.help) {
      ...
      if (options.open) {
        ...
        opn(`${protocol}://${host}:${options.port}`)
      }
  ...
  showConfig (options) {
    ...
    this.log(require('util').inspect(options, { depth: 99, colors: true }))
  }
