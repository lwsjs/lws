class Server {
  launch (options) {
    ...
    this.server = Server.getServer(options)
    const t = require('typical')
    if (t.isDefined(options['max-connections'])) this.server.maxConnections = options['max-connections']
    if (t.isDefined(options['keepalive-timeout'])) this.server.keepAliveTimeout = options['keepalive-timeout']
    ...
    this.server.on('listening', () => {
      const isHttps = options.https || options.http2
  ...
  static getServer (options) {
    ...
    if ((options.https || options.http2) && !(key && cert)) {
