...
class ServeCommand {
  ...
  partialDefinitions () {
    return [
      ...
      },
      {
        name: 'module-prefix',
        type: String,
        description: 'Prefix to when loading any module',
        section: 'extension'
      },
      {
        name: 'view',
  ...
  execute (options, argv) {
    ...
    if (options.help) {
      ...
      let Lws
      const LwsBase = require('../lws')
      const loadModule = require('load-module')
      if (options.https) {
        Lws = require('./https-server')(LwsBase)
      } else if (options.server) {
        Lws = loadModule(options.server)(LwsBase)
      } else {
        Lws = LwsBase
      }
      const lws = new Lws(options)
      lws.attachView(options)
      lws.launch(options)
